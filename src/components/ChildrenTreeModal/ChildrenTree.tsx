...
const ChildrenTree: React.FC<IChildrenTreeProps> = props => {
  const [children, setChildren] = useState<IChildRes[]>([]);
  const [colors, setColors] = useState<{ [key: string]: string }>({});

  useEffect(() => {
    if (!props.active) return;
    todoStore.getChildrenTree(props.parentId).then((res: IChildRes[]) => {
      res.forEach(el => {
        if (el.parent_task) {
          if (colors[el.parent_task]) return;
          setColors(prev => ({ ...prev, [el.parent_task!]: getRandomColor() }));
        }
      });
      setChildren(res);
    });
